		...
		for(WorkItem majorWork : majorWorks){
			...
			if(review!=null){
				...
				String params="?processId="+review.getWfProcessId()+"&orderId="+majorWork.getOrderId()
						+"&taskId="+majorWork.getTaskId()+"&reviewResultId="+review.getId()+"&flag="+review.getFlag()
...
/*	            result= function.getExportParameterList().getString("RESULT");//调用接口返回状态  
	...
	@Override
	public List<Map<String, Object>> getWorkgroupStandardSelect(
			Map<String, Object> condMap) {
		
		return commonDao.queryWorkgroupStandardList(condMap);
	}
