...
BEGIN		
	...
	select sum(p.qty* ifnull((select wp.standard_price from BMS_HR_WORKGROUP_PRICE wp 
	...
	order by wp.effective_date desc limit 1),0)) total_price, p.workshop
	...
	set cal_status=1;
	#删除小班组下计算月份下的工资
	delete from BMS_HR_PIECE_SALARY where factory=q_factory and workshop=q_workshop and workgroup=q_workgroup and team=q_team and work_date=q_month and salary_model='辅助人力';
	...
	left join TEAM_PRICE_SUM tps on tps.workshop=whs.workshop
	;
