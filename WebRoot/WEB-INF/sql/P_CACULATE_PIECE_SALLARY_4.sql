CREATE DEFINER=`bmsadmin`@`%` PROCEDURE `P_CACULATE_PIECE_SALLARY_4`(in q_factory varchar(100),in q_workshop varchar(100),in q_workgroup varchar(100),in q_team varchar(100),in q_bus_number varchar(5000),in q_start_date varchar(10), in q_end_date varchar(10))
BEGIN		
    ...
    set v_sql='create TEMPORARY  TABLE staff_hours_count select h1.* from BMS_HR_STAFF_PIECE_HOURS h1 where h1.salary_model=''自制件承包'' and h1.status in (''1'',''3'')';
	...
	select h.bus_number,sum(ifnull(h.distribution*h.work_hour,0)) total_real_hour,h.org_id,h.order_id
	...
	and find_in_set(bus_number,q_bus_number)>0  and salary_model='自制件承包';
	...
	ifnull(h.distribution*h.work_hour/tmp.total_real_hour,0) work_real_hour, ifnull(h.standard_price,0) standard_price,h.distribution,h.work_date,
	round(ifnull(h.distribution*h.standard_price*(ifnull(h.bus_count,1)+ifnull(h.bonus,0))*h.work_hour/tmp.total_real_hour,0),2) ppay,
	h.bus_number,h.factory,h.workshop,h.workgroup,h.team,ifnull(h.bus_count,1) bus_count,ifnull(h.bonus,0) bonus,'自制件承包','1',h.editor_id,h.edit_date,0,'',h.order_id
	from staff_hours_count h
	...
	where h.salary_model='自制件承包' and h.status in('1','3');
