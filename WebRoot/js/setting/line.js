...
$(document).ready(function(){
	...
	$(document).on("click","#btnAdd",function(){
		var dialog = $( "#dialog-add" ).removeClass('hide').dialog({
			width:600,
			modal: true,
			title: '<div class="widget-header"><h4 class="smaller"><i class="ace-icon fa fa-gear green"></i> 新增线别</h4></div>',
			title_html: true,
			buttons: [ 
				{
					text: "取消",
						...
						$.ajax({
						    ...
						    success:function(response){
						    	...
						    	ajaxQuery();
						    		...
						    		$.gritter.add({
										title: '系统提示：',
										text: '<h5>增加线别失败！</h5><br>'+response.message,
										class_name: 'gritter-info'
									});
						    	}
						    }
	...
	$(document).on("click",".editfactory",function(){
		...
		var dialog = $( "#dialog-edit" ).removeClass('hide').dialog({
			...
			/*height:500,*/
			modal: true,
			title: '<div class="widget-header"><h4 class="smaller"><i class="ace-icon fa fa-gear green"></i> 编辑线别</h4></div>',
			title_html: true,
			buttons: [ 
				{
					text: "取消",
					...
					$.ajax({
					    ...
					    success:function(response){
					    	...
					    	ajaxQuery();
					    		...
					    		$.gritter.add({
									title: '系统提示：',
									text: '<h5>编辑线别失败！</h5><br>'+response.message,
									class_name: 'gritter-info'
								});
					    	}
					    }
	...
	$.ajax({
	    ...
	    success:function(response){
	    	...
	    	ajaxQuery();
	    		...
	    		$.gritter.add({
					title: '系统提示：',
					text: '<h5>删除线别失败！</h5><br>'+response.message,
					class_name: 'gritter-info'
				});
	    	}
	    }
...
function ajaxQuery(){
	$("#tableData").dataTable({
		...
		columns: [
		            ...
		            {"title":"线别名称","class":"center","data":"lineName",/*"render": function ( data, type, row ) {
