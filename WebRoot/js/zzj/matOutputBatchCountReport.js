...
function ajaxQuery(){
	$("#tableResult").DataTable({
		...
		columns: [
		            ...
		            {"title":"需求种类","class":"center","data":"plan_quantity","defaultContent": ""},
		            {"title":"已完成种类","class":"center","data":"done_quantity","defaultContent": ""},
		            {"title":"欠产种类","class":"center","data":"","defaultContent": "","render":function(data,type,row){
		            	var html="";
		            	if(row.plan_quantity-row.done_quantity>0){
		            		html="<span onclick='showCountDetail(\""+row.zzj_type+"\",\""+row.batch+"\")' style='color:red;cursor: pointer;'>"+(row.plan_quantity-row.done_quantity)+"</span>"
		            	}else
...
function showCountDetail(zzj_type,batch){
	var obj = {id:"产量达成明细",title:"产量达成明细",close:'true',url: "/BMS/zzj/matOutputCountDetail?" +
			...
			batch+"&zzj_type="+zzj_type+"&order_id="+$("#search_order").attr("order_id")+
			"&order_no="+$("#search_order").val()
			,icon:''};
