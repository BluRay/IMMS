...
$(document).ready(function(){
	...
	$('#search_factory').change(function(){ 
		getWorkshopSelect("zzj/electrophoresisOuter",$("#search_factory :selected").text(),"","#search_workshop",null,"id");
	});
	...
	$(document).on("click","#btnAdd",function(){
		window.open("/BMS/zzj/electrophoresisAdd?business_type=1","_self");
	});
//	$(document).on("click","#btnAdd",function(){
//		
...
function ajaxQuery(){
	...
	var dt=$("#tableResult").DataTable({
		...
		columns: [
            ...
            {"title":"发货数量","class":"center","data":"quantity","defaultContent": ""},		            
            {"title":"发货日期","class":"center","data": "business_date","defaultContent": ""},
...
function initPage(){
	getFactorySelect("zzj/electrophoresisOuter","","#search_factory",null,"id");
	getWorkshopSelect("zzj/electrophoresisOuter",$("#search_factory :selected").text(),"","#search_workshop",null,"id");
	getLineSelect($("#search_factory :selected").text(),$("#search_workshop :selected").text(),'','#search_line','全部','name');
...
function edit(row){
	...
	//$("#edit_batch").val(row.batch!=undefined ?row.batch : "");
	...
	var dialog = $( "#dialog-edit" ).removeClass('hide').dialog({
		...
		height:450,
		modal: true,
		title: "<div class='widget-header widget-header-small'><h4 class='smaller'><i class='ace-icon glyphicon glyphicon-list-alt' style='color:green'></i> 编辑电泳件外发</h4></div>",
		title_html: true,
		zIndex: 1, 
		buttons: [ 
			{
