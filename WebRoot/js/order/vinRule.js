...
/*	getBusType($("#add_busType"));
	...
	getOrderNoSelect("#add_order","#orderId",function(obj){
		$("#add_area").val(obj.area)
		$("#add_area").trigger("change")
	});
	$(document).on("input","#add_order",function(){
		$("#add_order").attr("order_id","")
	})
...
/*	$("#btnDelete").on("click",function(){
	...
	*/
			...
			buttons: [ 
				...
				},
				{
					...
					/*	if($("#add_busType").val()===""){
						...
						if($("#add_order").attr("order_id")==undefined || $("#add_order").attr("order_id")===""){
							alert("请输入有效订单！");
							$("#add_order").val("").focus();
							return false;
						}
	...
	$(document).on("click",".editVinRule",function(){
		...
		$("#edit_order").val($(this).attr("order_no")).attr("order_id",$(this).attr("order_id"));
		$("#edit_order_desc").html($(this).closest('tr').find('td').eq(0).html())
		$("#edit_area").val($(this).closest('tr').find('td').eq(1).html());
		$("#edit_area").trigger("change")
/*		if($(this).closest('tr').find('td').eq(3).html().trim().length>0){
			$('#edit_wmiExtension').val($(this).closest('tr').find('td').eq(3).html());
			$('#edit_numberSize').val($(this).closest('tr').find('td').eq(4).html());
		}*/
		$('#edit_vinPrefix').val($(this).closest('tr').find('td').eq(2).html());
...
function ajaxQuery(){
	...
	$("#tableData").dataTable({
		...
		columns: [
		            ...
		            {"title":"编辑","class":"center","data":null,"defaultContent": "","render":function(data, type, row){
		            	var html="<i class=\"ace-icon fa fa-pencil bigger-130 editVinRule\" order_id='"+row.order_id+"' order_no='"+row.order_no+"' style='color:green;cursor: pointer;'></i>";
		            	return html;
		            }}
		          ],
