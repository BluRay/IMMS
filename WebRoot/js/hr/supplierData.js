...
$(document).ready(function(){
	...
	function drawTplDetailTable(tableId,data){
		...
		var tb=$(tableId).dataTable({
			...
			language: {
				emptyTable:"",					     
				infoEmpty:"",
				zeroRecords:"请导入供应商明细！"
			},
			data:data||{},
			columns: [
			          	{"title":"序号","class":"center","width":"8%","data":"item_no","defaultContent": ""},
			            {"title":"供应商名称","class":"center","width":"40%","data":"supplier","defaultContent": "","render":function(data,type,row){
			            	var html=data;
			            	if(row.exist_flag=='Y'){
			            ...
			            {"title":"供应商负责人","class":"center","width":"25%","data":"dutyman","defaultContent": ""},		
			            {"title":"联系电话","class":"center","width":"10%","data":"telephone","defaultContent": ""},
			            {"title":"备注","class":"center","width":"17%","data":"note","defaultContent": ""}
			          ]	
		});
	...
	function ajaxQuery(){
		...
		var columns=[
					...
					{"title":"外包供应商名称","class":"center","data":"supplier","defaultContent": ""},
	...
	function upload(form,file){
		...
		if (allowSubmit) {
			$("#uploadForm").ajaxSubmit({
				...
				url : 'uploadSupplierFile', // 需要提交的 url
				data : {
	...
	function ajaxSave(){
		...
		if(exist_span.length>0){
			save_flag=confirm("标红的供应商已经存在，是否更新？");
		}
		
		$(".divLoading").addClass("fade in").show();
		if(save_flag){
			$.ajax({
				url:"saveSupplierData",
				type:"post",
				dataType:"json",
				data:{
					supplier_list:JSON.stringify(json_tpl_list),
				},
				success:function(response){
					$(".divLoading").hide();
					alert(response.message);
